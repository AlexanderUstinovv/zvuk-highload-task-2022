docker_run = docker run --rm --mount type=bind,source="$(shell pwd)/",target=/root/ zvuk-highload-challenge:0.0.1
...
install-dependencies: ## Install challenge dependencies from Pipfile
	docker build --no-cache -t zvuk-highload-challenge:0.0.1 .
...
run: fetch-data ## Run challenge 
	$(docker_run) pipenv run python src/main.py uncommitted/input_logs.jsonl
